/* Prikaz 1 bloga iz JSON-a */
function prikazJednogBloga(){
  var test = document.getElementById("blog-post");
  console.log("cao1")

  $.getJSON('blogovi.json', function(data){
    var listaPostova = data.blogovi;
    
    function testTemplate(blog) {
      return `
      <div class="card mb-3">
        <div class="row g-0">
          <div class="col-md-4">
          ...
          <div class="card-body">
              <p class="card-text">
                <small class="text-muted">
                  <time datetime="2020-05-25 12:00:00"><i class="fas fa-calendar-alt mr-2"></i> ${formatiranjeDatuma(blog.datum)}</time>
                </small>
              </p>
              ...
              <a class="dugme1 my-3" href="blog-posts/blog-post-${blog.id}.html">Saznaj vise</a>
            </div>
          </div>
        </div>
      </div>
    ...
    ${listaPostova.map(testTemplate).join("")} `;
  })
  ...
  function duzinaFilter(objekat){
    ...
    function formatiranjeDatuma(objekat){
      ...
      // var noviDatum = Date.parse(RAWdatum)
      ...
      // console.log(datumOutput)
      return datumOutput
  
    }
}
// prikazJednogBloga(); - Blogovi Hard-Coded upisani u html fajlove
